...
# ---------------------------------------------------------------------------
# Closing the loop: derive gate configuration from shadow data.
#
# The Viveka Gate ships with base_rate_legit = 0.95, correct for the published
# stress benchmark (5% fraud) and badly wrong for a real accounts payable
# operation (~0.15%). At the wrong base rate the gate holds roughly 60% of
# legitimate payments and looks broken. Rather than swap one hardcoded guess for
# another, the shadow period measures it.
# ---------------------------------------------------------------------------

MIN_SHADOW_OBSERVATIONS = 500     # below this, do not override the shipped default
LAPLACE_PSEUDO_FRAUD = 1.0        # never claim a zero fraud rate from a finite sample


def derive_gate_config(shadow_df, base_config=None, verbose: bool = True):
    """
    Measure the deployment's own base rate from shadow observations and return a
    GateConfig carrying it. Falls back to the shipped default when the sample is
    too small to support an estimate.

    The Laplace correction matters here: a shadow window containing zero fraud
    would otherwise yield base_rate_legit = 1.0, telling the gate that fraud is
    impossible. One pseudo-event keeps the estimate honest about what a finite
    sample can support.
    """
    from gate import GateConfig
    cfg = base_config or GateConfig()

    n = len(shadow_df)
    if n < MIN_SHADOW_OBSERVATIONS:
        if verbose:
            print(f"[adhikara] {n} observations is below the {MIN_SHADOW_OBSERVATIONS} "
                  f"minimum; keeping shipped default base_rate_legit={cfg.base_rate_legit}")
        return cfg

    observed_fraud = float(shadow_df.fraud.sum())
    corrected_rate = (observed_fraud + LAPLACE_PSEUDO_FRAUD) / (n + LAPLACE_PSEUDO_FRAUD)
    measured = 1.0 - corrected_rate

    new_cfg = GateConfig(**{**cfg.__dict__, "base_rate_legit": measured})

    if verbose:
        print(f"[adhikara] measured base rate from {n:,} shadow observations: "
              f"{observed_fraud:.0f} fraud events, {corrected_rate*100:.3f}% "
              f"(Laplace corrected)")
        print(f"[adhikara] base_rate_legit: {cfg.base_rate_legit:.4f} "
              f"(shipped default) -> {measured:.4f} (measured)")
        if observed_fraud == 0:
            print("[adhikara] WARNING: no fraud observed in shadow window. The estimate "
                  "reflects the sample ceiling, not a demonstrated absence of fraud.")
    return new_cfg


def shadow_summary(shadow_df) -> dict:
    """Everything the shadow period measured, in one dict, for the customer report."""
    n = len(shadow_df)
    fraud_n = int(shadow_df.fraud.sum())
    return {
        "observations": n,
        "days": int(shadow_df.day.max() - shadow_df.day.min() + 1) if "day" in shadow_df else None,
        "fraud_events": fraud_n,
        "observed_fraud_rate": fraud_n / max(n, 1),
        "laplace_corrected_rate": (fraud_n + LAPLACE_PSEUDO_FRAUD) / (n + LAPLACE_PSEUDO_FRAUD),
        "sufficient_for_calibration": n >= MIN_SHADOW_OBSERVATIONS,
        "incumbent_holds": int(shadow_df.human_held.sum()) if "human_held" in shadow_df else None,
        "incumbent_caught": int((shadow_df.fraud & shadow_df.human_held).sum())
                            if "human_held" in shadow_df else None,
    }
